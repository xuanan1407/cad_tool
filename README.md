...
- **cad_processor.py**: Handles the extraction and analysis of geometric objects (rectangles, circles, polygons, etc.) from DXF files using ezdxf.
...
- Uses `CadProcessor` to extract rectangles, circles, and polygons from DXF files.
...
- Provides modular UI widgets:
	...
	- `FilterFrame`: Checkboxes to filter rectangles/circles/polygons.
...
- Extracts closed polygons (3+ vertices, excluding rectangles) from LWPOLYLINE and POLYLINE entities.
...
- Provides `highlight_object(x, y, all_data)` to highlight rectangles, circles, and polygons by drawing their outlines.
