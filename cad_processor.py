...
class CadProcessor:
    @staticmethod
    def extract_texts_from_dxf(msp):
        """Extract all TEXT entities with their string and position (strip, skip empty, print for debug)"""
        texts = []
        for text in msp.query("TEXT"):
            try:
                value = text.dxf.text.strip()
                if not value:
                    continue
                pos = text.dxf.insert
                texts.append({"text": value, "x": pos.x, "y": pos.y})
            except Exception:
                continue
        print("[DEBUG] Extracted TEXTs:")
        for t in texts:
            print(f"  Text='{t['text']}' at ({t['x']}, {t['y']})")
        return texts
    ...
    def is_rectangle(points, tolerance=RECTANGLE_TOLERANCE):
        ...
        return (
            abs(dists[0] - dists[1]) < tolerance
            and abs(dists[1] - dists[2]) < tolerance
