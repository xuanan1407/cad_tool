...
class FilterFrame:
    ...
    def _create_filter_frame(self):
        ...
        tk.Checkbutton(
            filter_frame,
            text="Show Rectangles",
            variable=self.show_rect,
...
class ButtonPanel:
    def __init__(
        self, parent, on_load_dxf, on_connect_cad, on_export, on_remove_duplicates=None
    ):
        self.parent = parent
        self._create_buttons(
            on_load_dxf, on_connect_cad, on_export, on_remove_duplicates
        )

    def _create_buttons(
        self, on_load_dxf, on_connect_cad, on_export, on_remove_duplicates=None
    ):
        ...
        tk.Button(
            btn_frame1,
            text="🔗 2. Connect AutoCAD",
            command=on_connect_cad,
            width=20,
            bg="#d4edda",
        ).pack(side=tk.LEFT, padx=5)
        tk.Button(
            btn_frame1, text="📊 3. Export Excel", command=on_export, width=15
        ).pack(side=tk.LEFT, padx=5)
        if on_remove_duplicates:
            tk.Button(
                btn_frame1,
                text="🧹 Remove Duplicates",
                command=on_remove_duplicates,
                width=18,
                bg="#ffe082",
            ).pack(side=tk.LEFT, padx=5)
...
class StatusBar:
    def __init__(self, parent):
        ...
        status_bar = tk.Label(
            parent, textvariable=self.status_var, bd=1, relief=tk.SUNKEN, anchor=tk.W
        )
